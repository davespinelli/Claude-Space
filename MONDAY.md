...
4. **X (Twitter)** — David: log in (existing or new @handle). Claude: post the daily scan + track-record chart each evening with the storefront link.
5. **Stripe** — David: account + two Payment Links ($99 Standard, $249 Pro, required custom field "Describe your strategy rules, tickers, period"). Paste URLs into research/build_site.py (Claude wires them).
After this, local scheduled tasks (Upwork bid run, X post, bounty check) take over and run whenever the desktop app is open; cloud routines cover everything that needs no login.
