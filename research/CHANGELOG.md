...
## 2026-09-23 — idea 2343 (lane C): IS THE KEEP-4b CANDIDATE A 200d ARTEFACT? THE MA-LENGTH LADDER. **ANSWERED = NO. THE PASS IS A PLATEAU, NOT A RIDGE. NO NEW CANDIDATE, NO RULES CHANGE.**
  ...
  **WHERE THIS COMES FROM.** Every 4b-passing book in this record — idea 2300's standing candidate
  (`RG100 + phi = 1.00`: every name inside the 200d +/-3% band at `gross / N_in`, idle NAV swept to
  SHY) included — is gated by a band around ONE moving-average length, L = 200 days, and L had
  never been swept. Idea 2241 priced the band's two EDGES and idea 2318 is sweeping its WIDTH c,
  both at L = 200; 2241's own entry lists "one MA length (200d)" as an honest limit. L sets how
  much of the past the gate averages and therefore how fast the book re-enters after a trough —
  exactly the channel the 4b CAGR floor binds on.

  **THE GRID.** **Two tuned dials and no more: L {100, 150, 200, 250, 300} and gross {0.75, 1.00}.**
  L = 200 IS the committed book, so one ladder spans it and its neighbours on both sides. Panels
  (U56 / B136 / SMALL), rungs {0, 10, 25, 50} bps, band c = 0.03, the weekly cadence, t+1 and the
  SHY sweep are REPORTED, never selected on. **30 books x 4 rungs = 120 published rows.**

  **THE ANSWER: ONLY THE TWO ENDS OF THE LADDER FAIL.** U56 at gross 0.75, 10 bps — L100
  11.54% / 1.0878 / -22.01% (4b FAIL on `L_DD`), L150 12.07% / 1.1557 / -17.77% PASS, **L200 (live)
  12.59% / 1.1934 / -17.39%, OOS 13.85% / 1.2397 PASS**, L250 12.59% / 1.1861 / -17.50% PASS, L300
  12.24% / 1.1341 / -17.91% PASS. SPY 15.23% / 0.8897 / -33.72%, so the DD cap is -20.23% and the
  CAGR floor 10.66%. B136 is the mirror image: L100 / L150 / L200 / L250 all pass and **L300 misses
  by 12 bps of drawdown** (-20.35% against -20.23%). The pass set is a CONTIGUOUS INTERIOR PLATEAU
  and both failures are `L_DD` alone. Turnover falls monotonically with L (U56 6.96 -> 3.81
  turns/yr) and so does max per-name weight (25.00% -> 12.50% at L = 250).

  **L = 200 IS NOT THE LADDER'S OPTIMUM.** `dOOS_Sharpe` against the live cell (U56, 0.75, 10 bps):
  -0.2506 / -0.1296 / 0 / -0.0416 / -0.0887 along L. L = 200 is the OOS Sharpe maximum on U56 but
  NOT on B136, where L = 250 wins (12.66% / 1.1204 against 12.29% / 1.0994). **U56 / L = 250 / 0.75
  is the only cell in all 120 rows clearing 4b at 50 bps** (10.84% / 1.0345 / -17.96%, OOS
  11.72% / 1.0518) — reported, not selected on; the 50 bps rung is a reported axis, so moving the
  book onto L = 250 would be tuning on it.

  **RULE 8 (L, gross fitted on <= 2016-12-31, 2017-2026 read ONCE). 0 OF 24 PICKS LAND ON L = 200.**
  Distribution **L150 15, L300 5, L100 3, L250 1, L200 0**. The IS choosers systematically take a
  SHORTER lookback than the committed one and pay for it out of sample: U56 at 10 bps the pick
  (L = 150) reads OOS **12.02% / 1.1101** against the incumbent's **13.85% / 1.2397** — -1.83 pp of
  OOS CAGR and -0.130 of OOS Sharpe. Picks beat SPY's OOS Sharpe at 15 of 24 and the live RULES v2
  book's at 4 of 24; 13 of 24 also pass 4b full-sample. **This is the run's honest caution: the
  plateau says the incumbent length is not FRAGILE, it does not say an out-of-sample chooser would
  have FOUND it.** L = 200 is a frozen convention that happens to sit near the top of its own
  ladder — a fact about the ladder, not evidence the chooser works.

  **BOTH KEEP PATHS. 4b 24, 4a 0 of 120.** By panel U56 13/40, B136 11/40, **SMALL 0 of 40 on both
  paths at every rung** (the same null 2322 and 2326 published for the cap and the floor); by rung
  8 / 8 / 7 / 1 at 0 / 10 / 25 / 50 bps. **Every 4b pass is at gross 0.75**; gross 1.00 fails
  `L_DD` at all 5 lengths on all 3 panels. Dominant binding leg on failures is `L_DD` (22-24 at
  every rung), then `L_CAGR` (5 -> 19 as costs rise). 4a at 0 repeats every prior run on this family.

  **THE WARM-IN QUESTION, HANDLED IN THE OPEN.** `band_state` is OUT before L closes exist, so
  L = 300 spends 39 days of the headline window forced all-out. Re-scoring the SAME return series
  on an equal-state window (index[360:], by which L = 300 has 60 days of live band state) moves 4b
  from 24 to **26 of 120**, with L200 6 -> 7 and L250 7 -> 8 and no verdict flipping against the
  long lengths. The headline window UNDERstates the long end; the ordering is not a warm-in artefact.

  **GATES 13 of 13.** G1 per-column replica == `engine.backtest` at **0.00e+00**. **G2
  `band_state_L(200)` == `baseline.band_state` at 0 differing cells** — the ladder's centre IS the
  live clause-2 object, not a re-implementation of it. G3 the L = 200 book == an independently
  constructed `RG100 + phi = 1` at 4.44e-16. G4 no leverage (max row sum 1.000000000 over 30 books).
  ...
  **G9 EXTERNAL REPRODUCTION of idea 2300's committed U56 headline** (12.55% / 1.1896 / -17.39%,
  ...
  the price-cache vintage residual idea 2326 published, not a construction difference.

  **SURVIVORSHIP, PUBLISHED NOT BURIED.** U56 and B136 are current-constituent lists and SMALL is a
  current-screen panel, so rule 9 flatters the CAGR side of every row here. It applies equally along
  the ladder, so the L-COMPARISON is cleaner than any LEVEL. Other honest limits: one band width
  (c = 0.03), one cadence (W), one phase, one delay (t+1), one sweep instrument, one re-gross
  exponent (phi = 1.00); flat costs with no spread, impact or borrow, which bites hardest at the
  high-turnover short-L end; five lengths on 50-day spacing, so the plateau is established at that
  resolution and no finer.

  **NO RULES CHANGE. KEEP L = 200 FROZEN.** Nothing on the ladder beats the incumbent cell at the
  headline rung, rule 8 lands on it at 0 of 24, and the one more cost-robust rung (L = 250) is worse
  on OOS Sharpe and reachable only by selecting on a reported axis. `RULES.md` / `scan.py` /
  `bot.py` / `baseline.py` untouched. Script
  `research/backtests/2026-09-23_ma-length-ladder-on-the-4b-candidate_C.py`, memo
  `...C.result.md`, artefacts `.grid.csv` (120 rows) / `.exposure.csv` / `.equalstate.csv` /
...
## 2026-09-22 — idea 2207 (lane C): IS THE KEEP-4b CANDIDATE A *REBALANCE-FREQUENCY* OBJECT? **ANSWERED = PARTIALLY — THE PASS SET MOVES WITH CADENCE, THE CORE CELL DOES NOT, AND CADENCE CREATES NOTHING**
  ...
  **WHERE THIS COMES FROM.** Every band x gross verdict in the record — 2119's ladder, 2121's
  chooser study, 2125's realised-gross reading, 2211's zero-parameter comparand — is priced
  WEEKLY, because that is the cadence RULES v2 happens to run. Nobody had counted the weekly
  convention as a dial. This run prices the identical 25-cell ladder at D / W / 2W / M.

  **THE GRID.** 2 panels x 25 cells (band {0,0.02,0.03,0.05,0.08} x gross {0.50..1.00}) x 4
  cadences = 200 books, each at 0 / 10 / 25 bps = **600 published rows**. Two tuned dials and no
  more: CADENCE and GROSS (the band is reported at all five rungs; 0 bps is the mechanism read
  only). **5 of 5 gates PASS** — G1 max|d| 0.000e+00 against `engine.backtest` at D/W/M, G3 the
  ladder's centre cell IS `baseline.rules_v2_weights` (max|d| 0.000e+00), G2 0 differing mask rows
  and |2W| 489 == ceil(977/2). Anchors reproduce the record: SPY U56 15.14% / 0.8851 / −33.72%,
  live RULES v2 (W) @10 bps 8.62% / 1.2010 / −12.05%, OOS 9.46% / 1.2767 / −12.05%.

  **THE ANSWER, PART 1 — THE SET MOVES.** 4b FULL passes of 25: U56 @10 bps **D4 / W5 / 2W5 / M3**,
  @25 bps D2 / W4 / 2W5 / M3; B136 @10 bps D1 / W4 / 2W4 / M2, @25 bps D1 / W1 / 2W0 / M2. Pass-set
  Jaccard against WEEKLY: **median 0.550**, range 0.000–1.000 over 12 panel x cost x cadence
  comparisons. Cells clearing 4b at ALL FOUR cadences: U56 **2** @10 bps / **1** @25 bps, **B136 0
  at either rung** — so the record's B136 band verdicts are cadence artefacts, two of them holding
  at exactly one cadence. Of the 14 weekly 4b passers, **26 of 42** cadence re-reads still pass.

  **THE ANSWER, PART 2 — BUT THE WEEKLY CONVENTION IS NOT CHERRY-PICKED.** 2W has the highest FULL
  Sharpe on every panel x cost (U56 @10 bps 1.2252 vs W 1.2011) and weakly dominates W on pass
  count and on 4b OOS passes (8 vs 6). Weekly is a CONSERVATIVE choice that leaves a little on the
  table, not a dial tuned to flatter the record.

  **MECHANISM: TWO DIFFERENT FAILURE MODES.** The CAGR floor is the sole binder at every cadence
  (18–24 of 25 cells), but the DD leg fail rate is **0.000 at D / W / 2W and 0.120 (U56) / 0.200
  (B136) at M**. At 0 bps the pass count is D5 / W5 / 2W5 / M3 (U56), so **the daily collapse is
  pure trading cost** (5 → 4 → 2 at 0 → 10 → 25 bps; B136 5 → 1 → 1) while **the monthly collapse
  is timing, present already at zero cost**. Turnover/yr D 3.17x / W 1.89x / 2W 1.51x / M 1.17x.

  **RULE 8, 2017–2026 READ ONCE.** Cadence x gross chosen on 2009–2016 IS rows only at the live
  band. All three legal choosers agree and pick `b0.03_g1.00` at the **M** cadence on U56: OOS
  12.82% / 1.2252 / −18.81%, 4b OOS PASS but **OOS rank 16 of 20** — the IS view prefers the
  cadence that spends drawdown. The zero-parameter MAXGROSS_W picks W and gets **2.9 pp less
  drawdown** (12.67% / 1.2760 / −15.91%). **Fitting premium for choosing cadence in-sample:
  −0.0075 OOS Sharpe**, extending 2211's zero-premium finding from the gross dial to the cadence
  dial. Fitted picks: 4b OOS 16 of 24, 4b FULL 19 of 24, **4a 0 of 24 in either window**.

  **ONE KEEP-4b CANDIDATE, RECORDED NOT ADOPTED.** U56 band 0.03 / gross 1.00 at W or 2W — the
  only cadence-invariant cell on the ladder (8 of 8 cadence x cost cells). W @10 bps FULL 11.53% /
  1.2009 / −15.91% (halves 1.2282 / 1.1799), OOS 12.67% / 1.2760 / −15.91%; 2W FULL 11.81% /
  1.2141 / −15.99%, OOS 13.10% / 1.3097 / −15.99%. It FAILS path 4a (drawdown −15.9% against the
  live book's −12.05%), its CAGR margin over the 4b floor is under 1.3 pp (inside the range
  2042 / 2090 found unresolvable at 95%), and its whole edge over the live book is the gross dial
  that 2211 showed needs no in-sample data. **NO RULES CHANGE.** RULES.md / PROTOCOL.md / scan.py /
  bot.py / baseline.py untouched. **Survivorship (rule 9):** U56 and B136 are current-constituent
  panels; every level here is optimistic and this is a within-tape cadence contrast only.
  Script `research/backtests/2026-09-22_band-ladder-rebalance-cadence_C.py`, results
  `..._C.result.md`, memo `..._C.memo.md`.
...
## 2026-09-22 — idea 2115 (lane C): IS THE REBALANCE-OFFSET SPREAD A COST ARTEFACT OR A PATH ARTEFACT? **ANSWERED, DECISIVELY — PATH, AT 162 OF 162 CELLS. KILL THE COST READING. NO NEW BOOK, NO RULES CHANGE.**
  ...
  **THE QUESTION.** Idea 914 killed a KEEP-4b candidate because its DD margin was smaller than the
  same book's spread across five weekly rebalance offsets, and idea 2119 (lane B, the same day)
  found only 7 of 16 ladder passes survive 5 of 5 offsets. Both treat the spread as given. If the
  spread were a turnover-TIMING BILL it must collapse toward zero at 0 bps and 914's clause would
  be a statement about the BROKER that better execution retires; if it is PATH SAMPLING it
  survives at zero cost and the clause is a statement about the RULE that nothing retires.

  **THE GRID.** Three of the record's own fixed shapes — BAND03_G075 (live RULES v2), TOP20 (the
  2026-09-04 first KEEP-4b shape) and EWELIG — x 2 panels x 4 cadences x 4 cost rungs x 5 offsets
  x 3 windows = **1,440 scored books, all published** (`.grid.csv.gz`). Exactly two tuned dials,
  COST RUNG and CADENCE; the offset is a MEASUREMENT and the reported book is ALWAYS d=0.

  **THE ANSWER.** Pre-registered buckets (COST if S(0) <= 0.10*S(50), PATH if S(0) >= 0.50*S(50)):
  **PATH 1.000 / MIXED 0.000 / COST 0.000** over 162 cells, and separately on all three legs
  (CAGR 54/54, Sharpe 54/54, MaxDD 54/54). **Median S(0)/S(10) = 1.0003** (min 0.889, max 1.100) —
  the WHOLE of PROTOCOL's own 10 bps offset spread is already there at ZERO cost. Slope |dS/dc|
  median **1.16e-05 per bp** against spread levels of 0.001-0.19 in the leg's own unit. B1c on the
  clip-free offsets {0,1,2} alone: PATH 0.988, median S0/S10 1.000.

  **AND THE ARITHMETIC AGREES — this is a split, not a fit.** Costs never touch the held path in
  this engine (gate G5, 0.000e+00), so `net_d(c) - net_0(c) = [gross_d - gross_0] - (c/1e4)*
  [turn_d - turn_0]` EXACTLY. At PROTOCOL's 10 bps **|COST_d|/|PATH_d| median 0.0072** — the
  weekday cost bill is seven tenths of one percent of the weekday path difference — and cost
  dominates in **0.0069** of 144 offset pairs (0.0358 / 0.0139 at 50 bps). By book 0.0036 /
  0.0065 / 0.0126; by cadence Q 0.0031, M 0.0067, W 0.0185.

  **IDEA 914's CLAUSE IS HARDENED, NOT RETIRED.** The cost rung does move how many books pass 4b
  (9 -> 6 -> 5 -> 2 d=0 passes at 0/10/25/50 bps) but that is the book's LEVEL dying of costs, not
  the spread shrinking. The share of d=0 4b passes whose EVERY leg margin exceeds its own offset
  spread runs **0.111 -> 0.000 -> 0.000 -> 0.000**: at PROTOCOL's own rung **zero of six** clear
  it and only two of six are 5-of-5-offset stable. The binding leg is DRAWDOWN at every rung.
  Because the spread is cost-invariant while the margins shrink with cost, **the clause can only
  get harder as execution assumptions get more honest, never easier** — the record cannot buy its
  way out of weekday fragility with better fills.

  **CAPITAL ARM — NO NEW KEEP.** Rule 8 with both dials chosen on IS 2009-2016 at d=0 by IS
  Sharpe, 2017-2026 read once; the cost rung is not a real choice (PROTOCOL rung 2 fixes it), so
  the headline picks CADENCE at 10 bps and the both-dials-free pick is a sensitivity only.
  Headline OOS **4b 1 of 6, 4a 2 of 6**; path 4a **10 of 216** d=0 cells (EWELIG 9/72,
  BAND03_G075 1/72, TOP20 0/72), path 4b 22 of 216, all TOP20. The single OOS 4b pass,
  `U56/TOP20/monthly @10bps` (FULL 14.70%/1.203/-19.51%, H1 1.211 H2 1.204; OOS
  16.68%/1.284/-19.51%; SPY OOS 15.29%/0.875/-33.72%; RULES v2 OOS 9.46%/1.277/-12.05%), is the
  record's **own 2026-09-04 shape at its month-end phase**, already de-certified by idea 879
  (7 of 21 monthly offsets at gross 0.75) and idea 956 (fails 16 of 21 DOM phases, pass-share
  0.238). Its OOS DD margin is **+0.723 pp against its own 2.258 pp OOS DD spread**, and this run
  proves that spread is 100% path. **Not proposed, no memo, rule 6 untouched;
  RULES.md / scan.py / bot.py / baseline.py unmodified.**

  **GATES 7/7 PASS.** G1 local `run()+net()` == `engine.backtest(W,10bps)` 0.000e+00; G2
  `offset_mask(.,0,R)` == `rebalance_mask(.,R)` 4/4 cadences 0 diffs; G3 BAND03_G075 ==
  `baseline.rules_v2_weights` 0.000e+00; G4 cadence D offset masks identical (the structural zero
  the D spread must be) 0 diffs; G5 cost linearity 0.000e+00, which is what makes the split exact;
  G6 clipping census published (clip-free {0,1,2}; W d3 = 2, d4 = 175) with B1 re-read on them as
  B1c rather than the bar quietly widened; G7 SPY offset-invariant by construction.

  **LIMITS, STATED.** SURVIVORSHIP (rule 9): U56 and B136 are current-constituent lists, so every
  absolute level is optimistic; this is a WITHIN-TAPE contrast — same names, same dates, only the
  rebalance weekday and the cost rung move — and it does not repair the level. The cost model is a
  flat bps-on-turnover bill, so a spread driven by weekday-varying SLIPPAGE rather than by
  turnover volume would be invisible here; the claim is about the record's own cost convention,
  which is the convention every 4b verdict was scored under. Cadence D is a structural zero and is
  excluded from every share above.
  `research/backtests/2026-09-22_offset-spread-cost-vs-path_C.py`
...
## 2026-09-23 — lane C run 60, idea 2495: noise in the decision price itself
- **THE RECORD HAS MOVED THE THRESHOLD EVERY WAY BUT ONE — IT HAD NEVER PERTURBED THE PRICE THE
  THRESHOLD IS READ FROM.** The standing candidate is a threshold rule on one number, the close
  against its own 200d MA. Band width, MA length, Donchian, MA slope and asymmetric entry/exit
  edges all move the THRESHOLD; leave-one-out and random name drops move the PANEL. A close is not
  an exact quantity — the auction print, a second vendor's adjusted close and a different
  back-adjustment convention disagree by basis points — and `band_state` is HYSTERETIC, so a
  spurious close RATCHETS a name into the book instead of washing out on the next bar.
- **DESIGN: DECISION-ONLY PERTURBATION.** `p~ = p x exp(sigma_k z)`, i.i.d. per name-day, seeded,
  k in {0, 10, 25, 50, 100, 200, 400} bps with 8 seeds at every k > 0, driving the band state, the
  eligible set, `N_in` and every weight. **P&L, SPY and the live RULES v2 baseline are earned on
  the TRUE tape at every k** — G7 reads the comparand spread at **0.000e+00 over all 3,136 rows** —
  so anything that moves is a decision movement and nothing else.
- **THE VERDICT IS INVARIANT AT EVERY PLAUSIBLE k. CONFIRM of the standing candidate.** The
  committed U56 CAP2 cell (cap 2%, g 0.75, weekly, 10 bps) passes 4b on **8 of 8 seeds at every k
  out to 200 bps and 7 of 8 at 400**; B136's on 8 of 8 including 400. Across the 25 of 64 cells
  passing 4b at k = 0: 4b survives in **100.0% of (cell x seed) draws at 10 and 25 bps, 99.5% at
  50, 96.0% at 100, 86.5% at 200, 63.5% at 400**; 15 of 25 never flip on any seed and the median
  first-flip k is **300 bps — larger than the 3% band itself**.
- **THE FRAGILITY MARGIN, PUBLISHED.** U56 committed cell at k = 0/10/25/50/100/200/400 bps: CAGR
  11.62 / 11.56 / 11.59 / 11.56 / 11.50 / 11.09 / 10.78%; Sharpe 1.2687 / 1.2631 / 1.2665 / 1.2643
  / 1.2579 / 1.2172 / 1.1899; MaxDD -14.81 / -14.81 / -14.80 / -14.75 / -14.75 / -14.62 / -14.36%;
  OOS Sharpe 1.3318 / 1.3263 / 1.3327 / 1.3358 / 1.3217 / 1.2872 / 1.2563. Out to 100 bps the whole
  eight-seed RANGE of Sharpe (1.2517..1.2760) lies within 0.02 of the committed value.
- **WHAT BREAKS IS TURNOVER, AND THE MECHANISM IS MEASURED NOT ASSUMED.** The same cell trades
  **3.51 / 3.52 / 3.55 / 3.64 / 4.00 / 5.50 / 9.34x per year** across k (+14% at 100 bps, +166% at
  400) for 0.84 pp of CAGR. Gate-state disagreement runs 0.25 / 0.70 / 1.48 / 3.25 / 5.67 / 9.38%
  and G12 reads the mean LENGTH of a disagreement at **4.8 / 5.7 / 6.6 / 8.0 / 6.2 / 3.0 trading
  days** — the hysteresis ratchet is real, worth about a week, and self-limiting once the noise
  exceeds the band. Realised risk gross moves by under 0.002 of NAV across the whole ladder, so
  this is a pure decision effect, not the exposure confound that killed twelve turnover devices.
- **THE CAVEAT THE RECORD SHOULD CARRY: THE RULE-8 PICK IS FAR MORE FRAGILE THAN THE VERDICT.**
  Over 784 picks, the (cap, gross) fitted on warm-up..2016-12-31 matches its k = 0 pick in only
  **91.4% of draws at 10 bps**, 85.2% at 25, 82.0% at 50, 80.5% at 100, 46.9% at 200, 35.9% at 400.
  About one committed rule-8 triple in twelve would have been a different triple under a 10 bps
  decision error, at a noise level at which not one 4b verdict flips. Every published rule-8 pick
  should be read as one draw from that distribution, not as a determinate choice. OOS of the picks:
  U56 12.81% / 1.2417 / -16.19% at k = 0, 12.71% / 1.2337 at 10 bps; B136 11.00% / 1.0484 /
  -17.36% and 11.01% / 1.0459; benchmarks read ONCE, SPY OOS 15.45% / 0.8831 / -33.72%, live RULES
  v2 OOS 9.51% / 1.2839 / -12.05%.
- **4a is 4 of 3,136 rows and all four appear ONLY at k = 400 bps on the cap-1.5% U56 arm** — noise
  artefacts, not candidates. The live book's -12.05% MaxDD stays unreachable by this family.
- 3,136 published rows, **14 of 14 gates**, incl. G3 reproducing the committed CAP2 and CAND U56
  headlines to **4.98e-05**, a bit-identical k = 0 `baseline.band_state` eligible set, an
  exact-linearity cost check (3.47e-18) and a replica matching `engine.backtest` to 0.000e+00.
- SURVIVORSHIP (rule 9): U56 / B136 are CURRENT constituents held from 2008; `L_CAGR` is the
