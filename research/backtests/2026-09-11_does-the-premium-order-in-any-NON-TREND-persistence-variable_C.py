#!/usr/bin/env python3
"""Idea 798 (lane C, 2026-09-11) - does-the-MA-gate-premium-order-in-ANY-persistence-variable-
that-is-NOT-a-TREND-statistic.

QUESTION
--------
Idea 569 found `tpers` (1 - daily flip rate of the name's own above-200d-MA state) is the first
characteristic to order the MA-gate SELECTION premium monotonically on kernel-matched k=36
draws (POOL slope +33.381187, R2 0.847210, slope*span +0.707607 = 7.24x the published GAP).
Idea 571 (lane B, yesterday) showed the R2 SURVIVES on three NON-GATE persistence variables
(mompers 0.7105, momsgn 0.7943, momac 0.7765 = 0.84-0.94x tpers's), so the ladder is not the
gate reading its own state - it reads PERSISTENCE.

But all four of those variables are TREND statistics: three are persistence of a 12-1 momentum
state and one is persistence of the 200d-MA state.  "Persistence" and "trend" are not separated
by any of them.  The queue's question: run the SAME kernel draws on persistence variables with
NO trend content and report whether the slope survives.

Three trend-free persistence variables are run beside tpers as the REFERENCE:

    tpers    REFERENCE   1 - flip rate of the above-200d-MA state.  TREND statistic, the gate's
                         own variable, idea 569's winner.  Carried only as the reproduction
                         anchor (G1) and the required comparand.
    retsgn   PRIMARY     1 - flip rate of the state "today's daily return > 0".  The queue's
                         named variable: SIGN PERSISTENCE OF DAILY RETURNS.  No lookback, no
                         level, no cross-sectional reference - a pure one-day sign statistic.
    volpers  PRIMARY     1 - flip rate of the state "vol20 > its own trailing 252d median".
                         The queue's named variable: VOL-REGIME STATE PERSISTENCE.  A second
                         moment, sign-free and direction-free, so it cannot carry trend.
    retac    ROBUSTNESS  lag-1 autocorrelation of the name's DAILY returns.  The continuous
                         reading of the same trend-free persistence, in case the binary
                         flip-rate form is what carries the ladder.

VOLUME-RANK PERSISTENCE (the queue's third named variable) is NOT RUNNABLE HERE and is reported
as such, never silently dropped: `baseline.load_volume` serves the SMALL panel only
(`load_volume(small=True)`; it raises for anything else), so the B136 half of the pool has no
volume cache in this sandbox and a volume characteristic could not be defined on the pooled name
set the draws are built from.  Building one is parked idea 429 (LOCAL/Actions, needs network).

DESIGN (idea 569/571's machinery, verbatim where it can be)
-----------------------------------------------------------
Pool  = 135 B136 tradables + SMALL tradables on the COMMON trading index.  SMALL names with
        ...
        dropna()'d exactly as it was, THEN the trend-free persistence columns are joined on that
        same index, so the pooled name set - and therefore every pre-registered rung level and
        every tpers draw - is bit-identical to idea 569's and to idea 571's.  That is what makes
        G1 a real reproduction gate rather than a re-estimate.
...
Draw  = k = 36 names without replacement, probability proportional to a Gaussian kernel on the
        name's own characteristic, w_i = exp(-0.5*((x_i-L)/h)^2), h = 0.5*sd(x) over the pooled
        names (idea 569's BW_MULT, NOT re-tuned).  Seeded crc32("CHAR|{char}|{L:.6f}|{flavour}|
        {seed}"), 6 seeds per rung.  k is 36 at every rung, so panel WIDTH is never confounded
        with composition.
Levels= PRE-REGISTERED as the pooled 10/30/50/70/90th percentiles of that characteristic.
Arms  = EWall (gross g over every priced tradable, CONTROL) and MA-RS (gross g over names above
        their 200d MA, RESPREAD, TREATMENT).  premium = Sharpe(MA-RS) - Sharpe(EWall) at the
        SAME (panel, g, cadence).  RESPREAD holds gross fixed: pure selection.
Flavours POOL / BONLY / SONLY, as idea 569, so H_CARRIER is re-measurable on the new variables.
        A rung runs only where the flavour can reach the level (feasible band = mean of its 36
        lowest / 36 highest names); infeasible rungs are reported, never silently dropped.

TUNED PARAMETERS (PROTOCOL rule 4, exactly two): the SIGNAL (which persistence variable) and the
target LEVEL.  Gross g in {0.50, 0.75, 1.00} and cadence {W, M} are REPORTED axes averaged over
for the headline premium (idea 51's convention); seed is replication; flavour is a reported
contrast.  EVERY grid point is written to .grid.csv.
...
R2_TPERS    = 0.847210  idea 569's committed tpers POOL R2   (the bar every variable is read against).
SLOPE_TPERS = 33.381187 idea 569's committed tpers POOL slope.
EFF_TPERS   = 0.707607  idea 569's committed tpers POOL slope*span.
R2_571      = {mompers 0.710500, momsgn 0.794300, momac 0.776500}  idea 571's committed off-gate
                        POOL R2s - the TREND-persistence band a trend-free variable is compared to.

H_INFO(char)   : the POOL premium is monotone in `char` in the required direction (sign fixed in
                 advance by the three REAL panels' own values of `char`, computed before any
                 premium is read), |slope*span| >= 0.5*GAP, AND R2 >= 0.5*R2_TPERS.
H_TRENDFREE    : at least one TREND-FREE persistence variable passes H_INFO.  Then the ladder is
                 a property of persistence per se and idea 571's reading is safe.
H_TREND        : tpers passes H_INFO and NO trend-free variable does.  Then the ladder is a
                 TREND-statistic fact, and "persistence carries the premium" (idea 571's Result 1)
                 has to be narrowed to "trend persistence carries the premium".
H_INDEP(char)  : |Spearman(char, tpers)| < 0.50 over the pooled names.  A trend-free variable that
                 passes H_INFO while being a near-copy of tpers is not independent evidence.
                 |Spearman(char, cvol)| is reported beside it: vol-regime persistence that is just
                 a vol level in disguise is not a persistence result either.
...
RULE 8 (walk-forward, required): WF-A refits every slope inside 2010-2016 and again in 2017-2026,
read once, and reports whether the sign holds and how the R2 ordering of the four variables
survives.  WF-B picks (char, level) by IS Sharpe ALONE at g=0.75/W on the POOL flavour, then reads
the OOS window once against RULES v2 on B136 and against SPY.  BOTH KEEP paths (4a vs RULES v2 on
the same panel, 4b vs SPY) are evaluated on EVERY book and counted.

Costs 10 bps per unit turnover, weights at close t applied t+1 (engine convention), no shorting,
no leverage.

GATES
-----
G1  idea 569's committed tpers POOL/BONLY/SONLY slopes, R2s and rung premia reproduce from this
    run's draws (the pooled name set, levels and seeds are built to be identical).
G2  fast_backtest == engine.backtest on a real book.
G3  idea 569's committed tpers matched-level B-S origin gaps reproduce.
G4  the trend-free variables are actually trend-free: |Spearman(char, mom_rank_persistence)| is
    reported for each, where mom_rank_persistence is idea 571's `mompers`, recomputed here.
"""
...
CADENCE = ["W", "M"]
MA_WIN = 200
MOM_LAG, MOM_LOOK = 21, 252         # 12-1 momentum (scan.py), used ONLY for the G4 trend check
VOL_WIN, VOL_MED = 20, 252          # volpers: 20d realised vol vs its own trailing 252d median
IS_END, OOS_START = "2016-12-31", "2017-01-01"
K = 36
SEEDS = [0, 1, 2, 3, 4, 5]
FLAVOURS = ["POOL", "BONLY", "SONLY"]
OLD_CHARS = ["cvol", "mrho", "beta", "tpers", "plevel"]   # idea 569's frame, for index parity
CHARS = ["tpers", "retsgn", "volpers", "retac"]
NEW_CHARS = ["retsgn", "volpers", "retac"]                # the TREND-FREE persistence variables
...
EFF_TPERS = 0.707607
R2_571 = {"mompers": 0.710500, "momsgn": 0.794300, "momac": 0.776500}
...
def fast_backtest(prices, weights, cost_bps=COST, freq="W"):
    """Vectorised equivalent of engine.backtest (asserted in G2).  Idea 312/568/569/571's runner."""
    idx = prices.index
...
def trendfree_name_chars(pool):
    """tpers (recomputed identically) + the three TREND-FREE persistence variables."""
    ma = pool.rolling(MA_WIN).mean()
    ...
    retsgn = _flip_persistence(r > 0.0, v_r)               # SIGN persistence of daily returns

    vol = r.rolling(VOL_WIN).std()
    med = vol.rolling(VOL_MED).median()
    v_v = vol.notna() & med.notna()
    volpers = _flip_persistence(vol > med, v_v)            # VOL-REGIME state persistence

    ac = {}
    for c in pool.columns:
        s = r[c].where(v_r[c]).dropna()
        ac[c] = float(s.corr(s.shift(1))) if len(s) > 250 else np.nan
    retac = pd.Series(ac, dtype=float)                     # lag-1 autocorr of DAILY returns
    return pd.DataFrame(dict(tpers=tpers, retsgn=retsgn, volpers=volpers, retac=retac))


def zero_share(pool):
    """Share of exactly-zero daily returns per name.  The pool is ffilled, so a stale/illiquid
    name gets runs of exact zeros, which READ AS PERSISTENCE on retsgn and retac.  Reported as a
    contamination control, never as a characteristic."""
    r = pool.pct_change()
    v = pool.notna() & pool.shift(1).notna() & r.notna()
    return ((r == 0.0) & v).sum() / v.sum().replace(0, np.nan)


def mompers_name(pool):
    """Idea 571's 12-1 momentum RANK persistence - the TREND persistence yardstick for G4."""
    mom = pool.shift(MOM_LAG) / pool.shift(MOM_LOOK) - 1.0
    rk = mom.rank(axis=1, pct=True)
    v = pool.notna() & mom.notna()
    return _flip_persistence(rk > 0.5, v)
...
def panel_chars(px, tradable, spy_r):
    ...
    nc = trendfree_name_chars(sub)
    out = dict(cvol=vol, breadth=breadth)
    for c in CHARS:
        out[c] = float(nc[c].mean())
    return out
...
def main():
    t0 = time.time()
    P("=" * 118)
    P("IDEA 798  does-the-MA-gate-premium-order-in-ANY-persistence-variable-that-is-NOT-a-TREND-"
      "statistic  (lane C, 2026-09-11)")
    P("=" * 118)
    ...
    P("Variables: tpers = the GATE'S OWN, a TREND statistic (REFERENCE, idea 569's winner);")
    P(f"           {NEW_CHARS} = persistence with NO TREND CONTENT.")
    P("VOLUME-RANK PERSISTENCE (the queue's third named variable) is NOT RUNNABLE in this sandbox:")
    P("  baseline.load_volume serves the SMALL panel only, so the B136 half of the pool has no")
    P("  volume cache and the characteristic cannot be defined on the pooled name set.  Parked")
    P("  idea 429 (LOCAL/Actions) builds that cache; this run reports 3 of the queue's 3 named")
    P("  variables minus that one, and says so rather than substituting a proxy.")
    P(f"Pre-registered: GAP {GAP:.4f}  ORIGIN568 {ORIGIN568:.4f}  tpers POOL R2 {R2_TPERS:.6f} "
      f"slope {SLOPE_TPERS:+.6f} effect {EFF_TPERS:+.6f}")
    P("  idea 571's committed off-gate TREND-persistence POOL R2 band: "
      + "  ".join(f"{k} {v:.4f}" for k, v in R2_571.items()))
    ...
    tf = trendfree_name_chars(pool)
    nc = tf.reindex(old.index)
    mompers_pool = mompers_name(pool).reindex(old.index)   # G4 yardstick only, never drawn on
    zshare_pool = zero_share(pool).reindex(old.index)      # contamination control, never drawn on
    ...
    # H_INDEP + G4: is a trend-free variable actually different, and actually trend-free?
    P("")
    P("  H_INDEP / G4 - cross-name Spearman against tpers (the GATE/trend variable), against")
    P("  cvol (the level control), and against mompers (idea 571's TREND persistence yardstick).")
    P("  Bar for independence: |rho| < %.2f." % INDEP_BAR)
    indep, H_INDEP = [], {}
    for ch_ in CHARS:
        rho = spearman(nc[ch_], nc["tpers"])
        pear = pearson(nc[ch_], nc["tpers"])
        rho_cvol = spearman(nc[ch_], old["cvol"])
        rho_mom = spearman(nc[ch_], mompers_pool)
        rho_z = spearman(nc[ch_], zshare_pool)
        ...
        indep.append(dict(char=ch_, spearman_vs_tpers=rho, pearson_vs_tpers=pear,
                          spearman_vs_cvol=rho_cvol, spearman_vs_mompers=rho_mom,
                          spearman_vs_zeroshare=rho_z, independent=H_INDEP[ch_],
                          trendfree=bool(abs(rho_mom) < INDEP_BAR) if ch_ != "tpers" else False))
        P(f"    {ch_:9s} vs tpers {rho:+.4f} (pearson {pear:+.4f})   vs cvol {rho_cvol:+.4f}   "
          f"vs mompers {rho_mom:+.4f}   vs zero-return share {rho_z:+.4f}   independent "
          f"{str(H_INDEP[ch_]) if ch_ != 'tpers' else '(self)'}")
    IND = pd.DataFrame(indep)
    g4 = IND[IND.char.isin(NEW_CHARS)]
    P(f"  G4  all three trend-free variables |rho vs mompers| < {INDEP_BAR:.2f}: "
      f"{bool((g4.spearman_vs_mompers.abs() < INDEP_BAR).all())}  "
      f"(max {g4.spearman_vs_mompers.abs().max():.4f})")
    ...
    for nm, (px, tr) in panels.items():
        ...
        P(f"    {nm:9s} tpers {ch['tpers']:.4f}  retsgn {ch['retsgn']:.4f}  "
          f"volpers {ch['volpers']:.4f}  retac {ch['retac']:+.4f}  "
    ...
    P("")
    P("=" * 118)
    P("H_INFO  - does the premium order in the characteristic when the characteristic has NO TREND?")
    P("=" * 118)
    slopes = []
    for ch_ in CHARS:
        for fl in FLAVOURS:
    ...
    P("")
    for ch_ in CHARS:
        tag = "TREND" if ch_ == "tpers" else "trend-free"
        P(f"  {ch_:9s} [{tag:10s}] H_INFO {str(H_INFO[ch_]):5s}   H_NOISE {str(H_NOISE[ch_]):5s}")
    H_TRENDFREE = any(H_INFO.get(c) is True for c in NEW_CHARS)
    H_TREND = bool(H_INFO.get("tpers") is True and not H_TRENDFREE)
    P("")
    P(f"  H_TRENDFREE (a TREND-FREE persistence variable clears the same bar): "
      f"{'PASS' if H_TRENDFREE else 'FAIL'}")
    P(f"  H_TREND (tpers clears it and no trend-free variable does): "
      f"{'PASS' if H_TREND else 'FAIL'}")
    ...
    if len(OR):
        ...
        for ch_ in CHARS:
            ...
            P(f"  {ch_:9s} H_CARRIER {str(H_CARRIER[ch_]):5s}   {int(d.within_floor.sum())}/{len(d)} "
              ...
              f"{mg/ORIGIN568:+.2f}x idea 568   mean |B-S| {float(d.gap.abs().mean()):.4f}   "
              f"mean |match resid| {float(d.match_resid.abs().mean()):.4f}")
    ...
    tpers_top_oos = bool(all(pw.loc["tpers", "r2_OOS"] >= pw.loc[c, "r2_OOS"]
                             for c in NEW_CHARS if c in pw.index))
    P(f"    tpers (the TREND variable) has the highest POOL R2: IS {tpers_top_is}, OOS {tpers_top_oos}")
    ...
    P("VERDICT")
    P("=" * 118)
    P("  " + f"{'char':9s} {'trend?':7s} {'R2':>7s} {'R2/569':>7s} {'|eff|/GAP':>10s} {'signOK':>7s} "
    ...
    for ch_ in CHARS:
        ...
        if not len(row):
            ...
            continue
        row = row.iloc[0]
        P("  " + f"{ch_:9s} {'TREND' if ch_ == 'tpers' else 'free':7s} {row['r2']:7.4f} "
    ...
    P("")
    if H_TREND:
        P("  ANSWER: NO - the ladder is a TREND-STATISTIC fact.  tpers clears the bar and NO")
        P("  trend-free persistence variable does, so idea 571's 'persistence in general' has to")
        P("  be narrowed to 'trend persistence'.")
    elif H_TRENDFREE:
        passers = [c for c in NEW_CHARS if H_INFO.get(c) is True]
        P(f"  ANSWER: YES - {passers} order the premium with no trend content at all.")
        P("  Check H_INDEP and the vs-mompers column above before reading that as independent evidence.")
    else:
        P("  ANSWER: NEITHER CLEARS THE BAR on this run's draws - tpers itself failed to reproduce")
        P("  its own H_INFO, which would be a G1 failure; read the gates above.")
    ...
    nc.join(mompers_pool.rename("mompers")).join(zshare_pool.rename("zero_return_share")).to_csv(
        OUT / f"{STAMP}.namechars.csv")
    P("")
